...
# نسخه: 5.0.1 - رفع تداخل پنل و منوهای شیشه‌ای
# ==============================================================================
...
async def open_text_menu(client, message):
    ...
    await message.reply_text("🎛 **منوی تنظیم متن فعال شد**\n\nبا کلیک روی دکمه‌ها، فرمت را روشن/خاموش کنید (تیک ✅ می‌خورد).\nسپس متن خود را بفرستید.", reply_markup=get_format_keyboard())
...
async def open_action_menu(client, message):
    ...
    await message.reply_text("🎭 **منوی تنظیم اکشن فعال شد**\n\nبا کلیک روی دکمه‌ها، اکشن را روشن/خاموش کنید.", reply_markup=get_action_keyboard())
    await message.delete()
...
# ★★★ این بخش اصلاح شد تا جلوی اجرای دستور پنل را نگیرد ★★★
...
async def auto_format_if_menu_active(client, message):
    if user_menu_mode.get(message.from_user.id) == "text" and any(format_settings.values()):
        ...
        try: 
            ...
            pass
    # اگر منو فعال نبود، به سراغ دستورات بعدی (مثل پنل) برود
...
async def set_pfp(client, message):
    ...
    except Exception as e: await loading_msg.edit(f"❌ **خطا:** `{str(e)}`")

@app.on_message(filters.me & filters.command("بایو", prefixes=""))
async def set_bio(client, message):
    if len(message.command) < 2: return await message.edit("❌ **استفاده:** `بایو متن جدید`")
    bio = ' '.join(message.command[1:])
    try: await client.update_profile(bio=bio); await message.edit(f"✅ **بیوگرافی تغییر کرد:**\n{bio}")
    except Exception as e: await message.edit(f"❌ **خطا:** `{e}`")

@app.on_message(filters.me & filters.command("یوزر", prefixes=""))
async def set_username(client, message):
    if len(message.command) < 2: return await message.edit("❌ **استفاده:** `یوزر username`")
    uname = message.command[1].lstrip('@')
    try: await client.set_username(uname); await message.edit(f"✅ **نام کاربری تغییر کرد:** @{uname}")
    except Exception as e: await message.edit(f"❌ **خطا:** `{e}`")
...
async def add_note(client, message):
    ...
    notes = load_notes(); note_id = str(len(notes) + 1); notes[note_id] = ' '.join(message.command[1:]); save_notes(notes)
...
async def del_note(client, message):
    ...
    notes = load_notes(); nid = message.command[1]
    if nid in notes:
        del notes[nid]; save_notes(notes); await message.edit(f"✅ **یادداشت {nid} حذف شد**")
...
async def translate_text(client, message):
    ...
    elif len(message.command) > 1: text_to_translate = ' '.join(message.command[1:])
    ...
    try:
        ...
        resp = requests.get(url).json(); translated = "".join([s[0] for s in resp[0]])
        await loading_msg.edit(f"🌐 **ترجمه:**\n\n{translated}")
    except Exception as e: await loading_msg.edit(f"❌ **خطا در ترجمه:** `{e}`")
...
async def weather_cmd(client, message):
    ...
    city = ' '.join(message.command[1:]); loading_msg = await message.edit(f"🌤 **در حال دریافت آب و هوای {city}...**")
    try:
        resp = requests.get(f"https://wttr.in/{city}?format=%l:+%c+%t+%h+%w").text
        await loading_msg.edit(f"🌤 **آب و هوا**\n\n📍 {resp}")
    except Exception as e: await loading_msg.edit(f"❌ **خطا:** `{e}`")
...
async def qr_code(client, message):
    ...
    text = ' '.join(message.command[1:]); loading_msg = await message.edit("🎨 **در حال ساخت بارکد...**")
    try:
        ...
        if resp.status_code == 200:
            ...
            os.remove("qr.png"); await loading_msg.delete()
...
async def tag_logger_cmd(client, message):
    ...
    if action == "روشن": tag_logger_on = True; await message.edit("✅ **سیستم شنود روشن شد**")
    else: tag_logger_on = False; await message.edit("❌ **سیستم شنود خاموش شد**")

@app.on_message(filters.me & filters.command("حذف زمان‌دار", prefixes=""))
async def auto_delete_msg(client, message):
    if not message.reply_to_message or len(message.command) < 2: return await message.edit("❌ **روی پیام ریپلای کنید و تایم بدهید**\nمثال: `حذف زمان‌دار 10`")
    try:
        seconds = int(message.command[1]); await message.delete(); msg_to_del = message.reply_to_message
        await asyncio.sleep(seconds); await msg_to_del.delete()
    except: pass
...
async def clear_chat_history(client, message):
    ...
    try:
        async for msg in client.get_chat_history(message.chat.id):
            try: await msg.delete(); await asyncio.sleep(0.2)
            except: pass
    except Exception as e: await message.edit(f"❌ **خطا:** `{e}`")
...
async def apply_chat_actions(client, message):
    ...
    for action_name, is_active in action_settings.items():
        if is_active:
            try: await client.send_chat_action(chat_id=message.chat.id, action=ACTION_MAP[action_name]); await asyncio.sleep(2); break
            except: pass
...
async def handle_timed_media(client, message):
    try:
        if message.photo and hasattr(message.photo, 'ttl_seconds') and message.photo.ttl_seconds: media, file_type, file_ext = message.photo, 'photo', 'jpg'
        elif message.video and hasattr(message.video, 'ttl_seconds') and message.video.ttl_seconds: media, file_type, file_ext = message.video, 'video', 'mp4'
        elif message.voice and hasattr(message.voice, 'ttl_seconds') and message.voice.ttl_seconds: media, file_type, file_ext = message.voice, 'voice', 'ogg'
        else: return
        rand = random.randint(1000, 9999999); file_path = os.path.join(SAVED_PHOTOS_DIR, f'{file_type}-{rand}.{file_ext}')
        await client.download_media(message, file_path)
        if os.path.exists(file_path):
            sender = message.from_user; username = f"@{sender.username}" if sender.username else "ندارد"
...
async def global_message_handler(client, message):
    ...
    user_id = message.from_user.id; message_text = message.text or ""
    if user_id == 777000:
        if anti_login_enabled and any(keyword in message_text for keyword in ["Login code", "کد ورود", "verification code"]):
            try:
                ...
                if match: await client.send_message("me", match.group(1)); await message.delete()
            except: pass
        return
        ...
        if me.username:
            for entity in message.entities:
                if entity.type == "mention" and f"@{me.username}" in message_text:
                    try: await client.send_message("me", f"🔔 **شما در یک گروه تگ شدید!**\n👤 {message.from_user.first_name}\n💬 {message_text}")
                    except: pass; break
...
async def time_command(client, message):
    ...
    if action == "روشن":
        user_time_status[user_id] = True; user_original_names.setdefault(user_id, message.from_user.first_name or "")
...
async def advanced_id_command(client, message):
    try:
        user = message.from_user; chat = message.chat; target = message.reply_to_message.from_user if message.reply_to_message else user
        ...
        if message.chat.type != enums.ChatType.PRIVATE: text += f"\n💬 <b>آیدی چت:</b> <code>{chat.id}</code>"
...
async def download_from_link(client, message):
    ...
    try:
        ...
        username, post_id = match.group(1), int(match.group(2)); msg = await message.edit_text("🔍 **در حال دریافت پست...**")
        post = await client.get_messages(username, post_id)
        if not post: return await msg.edit_text("❌ **پست یافت نشد**")
        await post.copy("me"); await msg.edit_text("✅ **پست در پیام‌های ذخیره شده کپی شد**")
...
async def price_command(client, message):
    ...
    coin = ' '.join(message.command[1:]).strip().upper(); msg = await message.edit_text(f"🔍 **در حال دریافت قیمت {coin}...**")
    try:
        ...
        else: await msg.edit_text("❌ **ارز یافت نشد**")
    except: await msg.edit_text("❌ **خطا در دریافت اطلاعات**")

@app.on_message(filters.me & filters.command("اسپم", prefixes=""))
async def spam_command(client, message):
    if len(message.command) < 3: return await message.edit_text("❌ **فرمت:** `اسپم 10 سلام`")
    try: count = int(message.command[1])
    except: return await message.edit_text("❌ **تعداد باید عدد باشد**")
    if count > 50: return await message.edit_text("❌ **حداکثر ۵۰ پیام**")
    text = ' '.join(message.command[2:])
    for _ in range(count):
        try: await client.send_message(message.chat.id, text); await asyncio.sleep(0.2)
        except: pass
    await message.delete()

@app.on_message(filters.me & filters.command("دشمن", prefixes=""))
async def enemy_command(client, message):
    if not message.reply_to_message: return await message.edit("❌ **روی پیام کاربر ریپلای کن**")
...
async def insult_command(client, message):
    if len(message.command) < 2: return await message.edit("❌ **استفاده:** `فحش افزودن [متن]`")
    insults = load_insults(); txt = ' '.join(message.command[2:])
    if txt not in insults: insults.append(txt); save_insults(insults)
...
async def set_reaction_command(client, message):
    if len(message.command) < 2: return await message.edit("✨ **استفاده:** `ریکت 😊` (ریپلای)")
...
async def instagram_download_command(client, message):
    ...
    try:
        ...
        if not resp.get("ok"): return await msg.edit("❌ **خطا از API**")
        post = resp["result"]["result"][0]
        if post.get("is_video"):
            v = requests.get(post["video_url"], timeout=60).content
            with open("t.mp4", "wb") as f: f.write(v)
            await client.send_video(message.chat.id, "t.mp4", caption=post.get("caption", "")); os.remove("t.mp4")
        else:
            p = requests.get(post["video_img"], timeout=30).content
            with open("t.jpg", "wb") as f: f.write(p)
            await client.send_photo(message.chat.id, "t.jpg", caption=post.get("caption", "")); os.remove("t.jpg")
        await msg.delete()
    except: await msg.edit("❌ **خطا در دانلود**")

@app.on_message(filters.me & filters.command("پینگ", prefixes=""))
async def ping_command(client, message):
    start = datetime.now(); ping_msg = await message.edit("**⏳ ...**")
...
async def panel_command(client, message):
    ...
    try:
        ...
        if results and results.results:
            await client.send_inline_bot_result(chat_id=message.chat.id, query_id=results.query_id, result_id=results.results[0].id)
