...
<code>تبچی</code> — وضعیت، آمار و شمارش دور بعدی
<code>تبچی کد 1</code> یا <code>تبچی کد 1,2,3</code> — انتخاب بنر(ها) برای چرخش
<code>تبچی شروع</code> — ارسال فوری یک دور
<code>تبچی تکرار 120 5</code> — هر ۱۲۰ ثانیه، ۵ دور
<code>تبچی تکرار 120 نامحدود</code> — چرخه‌ی نامحدود
<code>تبچی توقف</code> — توقف چرخه""",
...
<code>ویس کن</code> (ریپلای روی متن) — ویس به‌صورت ریپلای فرستاده می‌شود
<code>جمنای صدا زن</code> / <code>جمنای صدا مرد</code> — انتخاب صدا

<b>ویس ← متن:</b>
<code>متن</code> یا <code>تبدیل به متن</code> (ریپلای روی ویس/موزیک/ویدیو گرد) — زبان خودکار تشخیص داده می‌شود
<code>جمنای خودکار روشن</code> / <code>جمنای خودکار خاموش</code> — متن ویس‌های ورودی پیوی در Saved Messages

<b>آهنگ:</b>
<code>آهنگ نام خواننده و آهنگ</code> — جستجو و ارسال آهنگ
<code>آهنگ</code> (ریپلای روی ویس/آهنگ) — تشخیص اسم + ارسال نسخه کامل
<code>اسم آهنگ</code> (ریپلای روی ویس/آهنگ) — فقط اسم آهنگ و خواننده

⚙️ بخش هوش مصنوعی: از صفحه سوم پنل ← «🤖 هوش مصنوعی»""",

"ai": """🤖 <b>هوش مصنوعی (Gemini)</b>

<b>راه‌اندازی:</b>
<code>جمنای کلید XXXX</code> — ثبت API Key (از aistudio.google.com/apikey)
<code>جمنای وضعیت</code> — وضعیت کلید، مدل‌ها و ماژول‌ها
<code>جمنای مدل ها</code> — لیست مدل‌های در دسترس
<code>جمنای مدل NAME</code> / <code>جمنای مدل صدا NAME</code> — تغییر مدل متن/صدا
<code>جمنای ریست</code> — برگشت به تنظیمات پیش‌فرض

<b>استفاده:</b>
<code>جمنای سوالت</code> — پرسیدن از هوش مصنوعی
<code>جمنای ...</code> (ریپلای روی متن/عکس/ویس) — تحلیل، ترجمه، توضیح
<code>خلاصه</code> (ریپلای روی متن/ویس) — خلاصه‌سازی
<code>خلاصه 50</code> — خلاصه ۵۰ پیام آخر چت
<code>اصلاح</code> (ریپلای) / <code>اصلاح متن ...</code> — ویرایش املا و نگارش""",
